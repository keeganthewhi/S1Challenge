...
function profilListesi(fenomenler) {
  return fenomenler.map(fenomen => fenomen.profile);
}
...
function fenomenEkle(fenomenler, number, profile, followers, posts, platform) {
  fenomenler.push({ number, profile, followers, posts, platform });
  return fenomenler;
}
...
function fenomenGonderimSayisi(fenomenler, profile) {
  for (const fenomen of fenomenler) {
...
function platformaGoreCokGonderiYapanFenomen(fenomenler, platform) {
  const filteredFenomenler = fenomenler.filter(fenomen => fenomen.platform === platform && fenomen.posts !== "NA");
  const maxPostsFenomen = filteredFenomenler.reduce((maxFenomen, currentFenomen) => {
    return parseInt(currentFenomen.posts) > parseInt(maxFenomen.posts) ? currentFenomen : maxFenomen;
  });
  return maxPostsFenomen ? maxPostsFenomen.profile : "Platformda gönderi yapmış fenomen bulunamadı.";
}
